...
set terminal epslatex size 2.6, 2 standalone color solid 9
...
set xlabel '{\Large $x$ [\SI{}{mm}]}' offset 0,0.5
set xrange [-22:22]
set xtics 20 offset 0,0.1
set mxtics 2

set ylabel '{\Large $y$ [\SI{}{mm}]}' offset 0.8,0.0
set yrange [-22:22]
set ytics 20 offset 0.3,0
set mytics 2

set cblabel '{\Large $\phi_\text{solid}$ [\SI{}{-}]}' offset 0.2,0.0
set cbrange [0:1]
...
plot \
ifile u ($1*aj):($2*aj):($10+$11) w image t '',\
ifile using ($1*aj):($2*aj):($4*aa*aj):($5*aa*aj) ev 4:4:1:1 with vector lw 2 lc rgb 'forest-green' nohead title "",\
ifile using (($1+$4*aa)*aj):(($2+$5*aa)*aj):((-cs*$4+sn*$5)*aj*bb):(-( sn*$4+cs*$5)*aj*bb) ev 4:4:1:1 with vector lw 2 lc rgb 'forest-green' nohead title "",\
ifile using (($1+$4*aa)*aj):(($2+$5*aa)*aj):((-cs*$4-sn*$5)*aj*bb):(-(-sn*$4+cs*$5)*aj*bb) ev 4:4:1:1 with vector lw 2 lc rgb 'forest-green' nohead title "",\
