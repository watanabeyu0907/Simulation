...
void bicg_00(pois_fnc *pois, double **phir, double **phim)
{
	int ii, jj;
	int ex, ey;
	double hhxm, hhxp;
	...
	shared(para, ex, ey, pois, phir, phim)
	#endif
	for(jj=2; jj<ey; jj++){
	for(ii=2; ii<ex; ii++){
		...
		pois->xm[jj][ii] = para.idx*para.idx*(1.0-hhxm);
		pois->xp[jj][ii] = para.idx*para.idx*(1.0-hhxp);
		pois->ym[jj][ii] = para.idy*para.idy*(1.0-hhym);
		pois->yp[jj][ii] = para.idy*para.idy*(1.0-hhyp);
