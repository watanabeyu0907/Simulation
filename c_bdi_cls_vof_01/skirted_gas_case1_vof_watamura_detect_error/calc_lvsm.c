void calc_lvsm(double **lvs0, double **lvs3, double tht)
{
	...
	for(ii=2; ii<ex; ii++){
		...
		p3.xx = ((double)ii-1.5)*para.dx - 0.5*para.lx*para.nx/(double)(para.nx-0);
		p3.yy = ((double)jj-1.5)*para.dy - 0.5*para.ly*para.ny/(double)(para.ny-0);
		
		p0.xx =  p3.xx*cs + p3.yy*sn + 0.5*para.lx*para.nx/(double)(para.nx-0);
		p0.yy = -p3.xx*sn + p3.yy*cs + 0.5*para.ly*para.ny/(double)(para.ny-0);
