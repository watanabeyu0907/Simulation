void init_phim(double **phi)
{
	...
	for(ii=0; ii<ex; ii++){
		...
		// @ cell centre
		dis_xm = ((double)ii - 2.0)*para.dx - 0.5*para.lx*para.nx/(double)(para.nx-0) - 0.5*para.lx*para.ar;
		dis_xp = ((double)ii - 1.0)*para.dx - 0.5*para.lx*para.nx/(double)(para.nx-0) - 0.5*para.lx*para.ar;
		dis_ym = ((double)jj - 2.0)*para.dy - 0.5*para.ly*para.ny/(double)(para.ny-0);
		dis_yp = ((double)jj - 1.0)*para.dy - 0.5*para.ly*para.ny/(double)(para.ny-0);
