void init_phif(double **phir, double **phim, double **phif)
{
	...
	radi_in = radi*(para.nx-0-para.bet*2)/(para.nx-0);
	radi_out = radi*(para.nx-0+para.bet*2)/(para.nx-0);
	ni = nj = 16;
	...
	for(ii=0; ii<para.nx+4; ii++){
		dis_xm = ((double)ii - 2.0)*para.dx - 0.5*para.lx*para.nx/(double)(para.nx-0);
		dis_xp = ((double)ii - 1.0)*para.dx - 0.5*para.lx*para.nx/(double)(para.nx-0);
		dis_ym = ((double)jj - 2.0)*para.dy - 0.25*para.ly*para.ny/(double)(para.ny-0);
		dis_yp = ((double)jj - 1.0)*para.dy - 0.25*para.ly*para.ny/(double)(para.ny-0);
