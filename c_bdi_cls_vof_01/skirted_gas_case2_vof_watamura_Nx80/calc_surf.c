void calc_surf(double **phir, double **phim, double **lvs0, double **lvm3, double **lvf3, coord *rnf, coord *rns, coord *rnfc, coord *rnfa, coord *rnsc, coord *rnsa, double **phit3, coord *sft)
{
	...
	for(ii=2; ii<ex1; ii++){
		...
		sft->xx[jj][ii]*= para.bet/(2.0*pow(cosh(para.bet*0.5*(lvf3[jj][ii]+lvf3[jj][ii+1])),2.0));
	...
	for(ii=2; ii<ex2; ii++){
		...
		sft->yy[jj][ii]*= para.bet/(2.0*pow(cosh(para.bet*0.5*(lvf3[jj][ii]+lvf3[jj+1][ii])),2.0));
