...
set terminal epslatex size 2.8, 2 standalone color solid 9
...
# set contour base
# set cntrparam levels discrete -15,-10,-5,5,10
# unset surface
...
set xlabel '{\Large $x$ [\SI{}{m}]}' offset 0,0.5
set xrange [-0.50:0.50]
set xtics 0.5 offset 0,0.1
set mxtics 2

set ylabel '{\Large $y$ [\SI{}{m}]}' offset 0.8,0.0
set yrange [-0.5:0.5]
set ytics 0.5 offset 0.3,0
set mytics 2

set cblabel '{\Large $\phi$ [\SI{}{-}]}' offset 0.7,0.0

set cbrange [-20:10]
set cbtics 5 offset -0.3,0
# set mcbtics 5

set palette defined (\
    0     0.2667 0.0039 0.3294,\
    0.25  0.2314 0.3216 0.5451,\
    0.5   0.1294 0.5686 0.5490,\
    0.75  0.3686 0.7882 0.3843,\
    1     0.9922 0.9059 0.1451)
...
# aaa = sprintf('{\Large $t = $ %4.2f [\SI{}{s}]}',nn/fps)
...
plot \
ifile u ($1*aj):($2*aj):22 w image t '',\
...
ifile using (($1+$4*aa)*aj):(($2+$5*aa)*aj):((-cs*$4-sn*$5)*aj*bb):(-(-sn*$4+cs*$5)*aj*bb) ev 8:8:1:1 with vector lw 2 lc rgb 'forest-green' nohead title "",\
'cont.dat' u ($1*aj):($2*aj) w l lw 2 lc rgb 'white' t ''

# 'cont1.dat' u ($1*aj):($2*aj) w l lw 2 lc rgb 'red' t '',\
# 'cont2.dat' u ($1*aj):($2*aj) w l lw 2 lc rgb 'black' t ''
